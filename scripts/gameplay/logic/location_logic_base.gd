class_name LocationLogicBase
extends Resource

var _contexts := GameServices.contexts


func get_start_of_turn_resolvable() -> BaseResolvable: return null
func get_start_of_turn_power(location: Location) -> LocationPower: return null
func get_end_of_turn_resolvable() -> BaseResolvable: return null
func get_end_of_turn_power(location: Location) -> LocationPower: return null
func get_to_close_resolvable() -> BaseResolvable: return null
func get_when_closed_resolvable() -> BaseResolvable: return null
