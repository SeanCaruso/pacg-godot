...
func test_voidglass_armor_can_recharge_for_any_damage():
	...
	GameServices.contexts.new_resolvable(DamageResolvable.new(valeros, 1, "Magic"))
	...
	assert_true(GameServices.contexts.current_resolvable.can_commit(GameServices.asm.staged_actions), "Should be able to commit")
...
func test_voidglass_armor_can_display_then_recharge_for_any_damage():
	...
	GameServices.contexts.new_resolvable(DamageResolvable.new(valeros, 1, "Special"))
...
func test_voidglass_armor_prompts_on_mental_damage_when_displayed():
	...
	GameServices.contexts.new_resolvable(DamageResolvable.new(valeros, 3, "Mental"))
...
func test_voidglass_armor_mental_damage_power_allows_recharge():
	...
	assert_true(GameServices.contexts.current_resolvable is PlayerChoiceResolvable, "Should have player choice")
	...
	assert_eq(damage_resolvable, GameServices.contexts.current_resolvable, "Should return to damage resolvable")
	assert_false(GameServices.contexts.current_resolvable.can_commit(GameServices.asm.staged_actions), "Should not be able to commit without more actions")
	
	var damage_actions := GameServices.contexts.current_resolvable.get_additional_actions_for_card(test_longsword)
	...
	assert_true(GameServices.contexts.current_resolvable.can_commit(GameServices.asm.staged_actions), "Should be able to commit now")
